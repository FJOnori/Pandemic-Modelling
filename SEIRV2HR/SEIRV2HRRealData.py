...
class SEIRSV2_Model():
    ...
    def __init__(self):
        ...
        self.RecoveryRate       = 1/14
        self.ImmunityLossRate   = 1/210
        self.Vaccination1Rate   = np.repeat( np.concatenate((np.array([0]*313), np.array([1/100]*787))), self.dt**(-1) )
        self.Vaccination2Rate   = np.repeat( np.concatenate((np.array([0]*313), np.array([1/100]*787))), self.dt**(-1) )
        self.Vaccination1HRRate = np.repeat( np.concatenate((np.array([0]*313), np.array([1/50]*787))), self.dt**(-1) )
        self.Vaccination2HRRate = np.repeat( np.concatenate((np.array([0]*313), np.array([1/50]*787))), self.dt**(-1) )

        self.ContactFactor      = 1.1
    ...
    def NumInt(self):
        ...
        for n in range(0,self.Iterations):
            ...
            dS = -NewInfections + self.ImmunityLossRate*R[n] - self.Vaccination1Rate[n]*S[n] + self.Population*self.BirthRateLR - self.DeathRate*S[n]
            ...
            dR = self.RecoveryRate*I[n] - self.ImmunityLossRate*R[n] - self.Vaccination1Rate[n]*R[n] - self.DeathRate*R[n]

            dV1 = self.Vaccination1Rate[n]*S[n] - self.Vaccination2Rate[n]*V1[n] - NewInfectionsV1 - self.DeathRate*V1[n] + self.ImmunityLossRate*RV1[n]
            ...
            dRV1 = self.Vaccination1Rate[n]*R[n] + self.RecoveryRate*IV1[n] - self.ImmunityLossRate*RV1[n] - self.Vaccination2Rate[n]*RV1[n] - self.DeathRate*RV1[n]

            dV2 = self.Vaccination2Rate[n]*V1[n] - NewInfectionsV2 - self.DeathRate*V2[n] + self.ImmunityLossRate*RV2[n]
            ...
            dRV2 = self.Vaccination2Rate[n]*RV1[n] + self.RecoveryRate*IV2[n] - self.ImmunityLossRate*RV2[n] - self.DeathRate*RV2[n]

            dSHR = -NewInfectionsHR + self.ImmunityLossRate*RHR[n] - self.Vaccination1HRRate[n]*SHR[n] + self.Population*self.BirthRateHR - self.DeathRate*SHR[n]
            ...
            dRHR = -self.ImmunityLossRate*RHR[n] + self.RecoveryRate*IHR[n] - self.Vaccination1HRRate[n]*RHR[n] - self.DeathRate*RHR[n]

            dV1HR = self.Vaccination1HRRate[n]*SHR[n] - self.Vaccination2HRRate[n]*V1HR[n] - NewInfectionsV1HR - self.DeathRate*V1HR[n] + self.ImmunityLossRate*RV1HR[n]
            ...
            dRV1HR = self.Vaccination1HRRate[n]*RHR[n] + self.RecoveryRate*IV1HR[n] - self.ImmunityLossRate*RV1HR[n] - self.Vaccination2HRRate[n]*RV1HR[n] - self.DeathRate*RV1HR[n]

            dV2HR = self.Vaccination2HRRate[n]*V1HR[n] - NewInfectionsV2HR - self.DeathRate*V2HR[n] + self.ImmunityLossRate*RV2HR[n]
            ...
            dRV2HR = self.Vaccination2HRRate[n]*RV1HR[n] + self.RecoveryRate*IV2HR[n] - self.ImmunityLossRate*RV2HR[n] - self.DeathRate*RV2HR[n]

            self.PopulationLR += (dS+dE+dI+dR+dV1+dV2+dEV1+dEV2+dIV1+dIV2+dRV1+dRV2)*dt
            self.PopulationHR += (dSHR+dEHR+dIHR+dRHR+dV1HR+dEV1HR+dIV1HR+dV2HR+dEV2HR+dIV2HR+dRV1+dRV2+dRV1HR+dRV2HR)*dt
            self.Population = (self.PopulationLR + self.PopulationHR)
            
            time.append(n * self.dt)
            Deaths.append((self.DeathRateCOVID*I[n] + self.DeathRateCOVIDSV*IV1[n] + self.DeathRateCOVIDDV*IV2[n] + self.DeathRateCOVIDHR*IHR[n] + self.DeathRateCOVIDSVHR*IV1HR[n] + self.DeathRateCOVIDDVHR*IV2HR[n])*self.dt)
        ...
        plt.title("COVIDGE Model Lineplot, CF = " + str(self.ContactFactor))
        plt.xlabel("Time (Days)")
        plt.ylabel("Proportion of Population")
        plt.grid(ls=":", c="grey", axis='y')
        plt.savefig("COVIDGEMultipliedCoefficients - " + str(self.ContactFactor) + " - " + str(Deaths[-1]*67000000) +  ".png", dpi=227)
