...
class SEIRSV2_Model():
    ...
    def NumInt(self):
        ...
        S = [self.PopulationLR - self.InitalExposuresLR]
        E = [self.InitalExposuresLR]
        I = [0]
        R = [0]
        ...
        SHR = [self.PopulationHR - self.InitalExposuresHR]
        EHR = [self.InitalExposuresHR]
        IHR = [0]
        RHR = [0]
        ...
        for n in range(0,self.Iterations):
            ...
            V1.append(V1[n] + dV1*dt)
            EV1.append(EV1[n] + dEV1*dt)
            IV1.append(IV1[n] + dIV1*dt)

            V2.append(V2[n] + dV2*dt)
            EV2.append(EV2[n] + dEV2*dt)
            IV2.append(IV2[n] + dIV2*dt)

            SHR.append(SHR[n] + dSHR*dt)
            EHR.append(EHR[n] + dEHR*dt)
            IHR.append(IHR[n] + dIHR*dt)
            RHR.append(RHR[n] + dRHR*dt)

            V1HR.append(V1HR[n] + dV1HR*dt)
            EV1HR.append(EV1HR[n] + dEV1HR*dt)
            IV1HR.append(IV1HR[n] + dIV1HR*dt)

            V2HR.append(V2HR[n] + dV2HR*dt)
            EV2HR.append(EV2HR[n] + dEV2HR*dt)
            IV2HR.append(IV2HR[n] + dIV2HR*dt)


        plt.plot(np.array(S) + np.array(SHR), label = "Susceptible", c = '#1e68b3')
        plt.plot(np.array(E) + np.array(EV1) + np.array(EV2) + np.array(EHR) + np.array(EV1HR) + np.array(EV2HR), label = "Exposed", c = '#eb781a')
        plt.plot(np.array(I) + np.array(IV1) + np.array(IV2) + np.array(IHR) + np.array(IV1HR) + np.array(IV2HR), label = "Infected", c = '#b81111')
        plt.plot(np.array(R) + np.array(RHR), label = "Recovered", c = '#aaacad')
        plt.plot(np.array(V1) + np.array(V1HR), label = "Vaccinated", c = '#5e017d')
        plt.plot(np.array(V2) + np.array(V2HR), label = "Fully Vaccinated", c = '#439603')
